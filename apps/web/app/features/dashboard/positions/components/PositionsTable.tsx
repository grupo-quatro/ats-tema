...
function StatusChip({ status }: { status: Job["status"] }) {
    const color = status === "open" ? "success" : status === "draft" ? "default" : status === "paused" ? "warning" : "default";
    const label = status === "open" ? "Abierta" : status === "paused" ? "Pausada" : status === "draft" ? "Borrador" : "Cerrada";
    return <Chip label={label} color={color as any} size="small" />;
}

export default function PositionsTable({ jobs }: Props) {
    return (
        <TableContainer component={Paper} sx={{ mt: 3, borderRadius: 3, boxShadow: 3, overflow: 'hidden' }}>
            <Table sx={{ minWidth: 650 }}>
                <TableHead>
                    <TableRow sx={{ bgcolor: 'primary.main' }}>
                        <TableCell sx={{ color: 'white', fontWeight: 700 }}>Posición</TableCell>
                        ...
                        <TableCell sx={{ color: 'white', fontWeight: 700 }}>Candidatos</TableCell>
                        <TableCell sx={{ color: 'white', fontWeight: 700 }}>Estado</TableCell>
                        <TableCell sx={{ color: 'white', fontWeight: 700 }}>Publicada</TableCell>
                ...
                <TableBody>
                    {jobs.map((job) => (
                        <TableRow key={job.id} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
                            ...
                            <TableCell>
                                <Chip label={job.department} variant="outlined" size="small" sx={{ borderRadius: 2 }} />
                            </TableCell>
                            <TableCell>{job.city ? `${job.city}` : job.location}</TableCell>
                            <TableCell>{job.type ?? '—'}</TableCell>
                            <TableCell>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, color: 'primary.main' }}>
                                    <Users size={14} /> <Box component="span">{job.candidates ?? 0}</Box>
                                </Box>
                            </TableCell>
                            <TableCell>
                                <Box sx={{ display: 'inline-flex', alignItems: 'center' }}>
                                    <Box sx={{ bgcolor: '#d1fae5', color: '#065f46', px: 1.2, py: 0.4, borderRadius: '999px', fontWeight: 700, fontSize: 12 }}>
                                        {job.status === 'open' ? 'Abierta' : job.status === 'paused' ? 'Pausada' : job.status}
                                    </Box>
                                </Box>
                            </TableCell>
